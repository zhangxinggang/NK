...
(function() {
        ...
        if(self.options.streamFile && self.options.streamFile.saveLocal){
            ...
            utils.spawnFfmpeg([
                ...
                function(err){
                    ...
                    var startTimePath=path.join(self.options.streamFile.savePath,(self.options.optFileStartTime+'.'+videoType));
                    var lastFileName=self.options.optFileStartTime +'_'+moment(new Date()).format('HH-mm-ss')+'.'+videoType;
                            ...
                            if(err){
                                ...
                                utils.avToMp4Ffmpeg(endTimePath,'mp4',function(err){
                                    if(err){
                                        ...
                                        var name=times+'.mp4';
                                        videoDB.get('data').push({
                                            time:times,
                                            name:name,
                                            url:path.join(self.options.liveSourceHead,(self.options.deviceName+'/'+localDir+'/'+name)).replace(new RegExp(/\\/,'g'),'/')
                                        }).write();
        ...
        if(options.pushStream && options.pushStream.open){
            ...
            for(var i=0;i<options.pushStream.streamPath.length;i++){
                if(!self.pushStream[options.pushStream.streamPath[i]]){
                    var tempPushStream=child_process.spawn("ffmpeg", ["-i", options.streamUrl, '-codec', 'copy', '-f', 'rtsp',options.pushStream.streamPath[i]],{
                        detached: false
                    });
    ...
    Mpeg1Muxer.prototype.liveStreamFun=function(){
        ...
        if(options.streamFile && options.streamFile.live){
            ...
            var cmd=["-i", options.streamUrl,'-fflags','flush_packets','-max_delay','1','-an','-flags','-global_header',
            '-hls_time','1','-hls_list_size','3','-hls_wrap','3','-vcodec','copy','-y',defPath
            ];
            utils.spawnFfmpeg(cmd,{detached:true},function(param,data){
                // return self.emit('mpeg1data',data);
            },function(err){
                if(err){
                    console.log(err)
    ...
    Mpeg1Muxer.prototype.resetStreamFileSavePath=function(){
        ...
        if(folderClassifiyAsTime[0]){
            ...
            this.options.streamFile.savePath=path.join(this.options.streamFile.originalPath,this.options.deviceName,strTime);
        }else{
