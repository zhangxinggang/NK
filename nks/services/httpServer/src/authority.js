...
let verifyToken=async (ctx,next)=>{
	...
	if(reqUrl=='/' && httpServer.homePage_url){
		ctx.redirect(httpServer.homePage_url);
		return;
	}
	...
	if(noAuthorityRoutes.length>0){
		...
		let token=ctx.request.body.access_token || ctx.request.query.access_token || ctx.headers[projectCookies.token] || ctx.cookies.get(projectCookies.token);
		if(token){
			try{
			...
			ctx.formatError([403,new Error('No token provider!')])
		}
	}
}
export {
