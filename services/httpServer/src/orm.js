...
const ormConn=function(options={},entitySchema){
	...
	ormPool[ormConnStr]['authenticate']=function(){
		return new Promise(function(resolve,reject){
			typeorm.createConnection(ormConf).then(function(res){
				res.models={};
...
const formatterQueryParam=function(sender,orm){
    ...
    filterRules=querys.includes('filterRules')?options.query.filterRules:(bodys.includes('filterRules')?options.body.filterRules:'');
	if(filterRules){
		for(var i=0;i<filterRules.length;i++){
			if(filterRules[i]['op']=='contains'){
				filterRules[i]['value']="%"+filterRules[i]['value']+"%";
			}
			queryObj['where'][filterRules[i]['field']]=typeorm[filterRuleKey[filterRules[i]['op']]](filterRules[i]['value'])
		}
	}
...
const otbExec=function(options,orm){
	...
	if(currencyOperation.view.includes(action)){
		...
		if(options['isTree']){
			if(queryObj['select']){
				if(typeof(queryObj['select'])=='object'){
					queryObj['select']=queryObj['select'].map(function(item){
						return 'o.'+item
					}).join(',');
				}
			}else{
			...
			if(options['idParentField']){
				let selfToSelfQb=orm.getRepository(options['tableName'])
					...
					if(queryObj['leftJoin']){
						for(let key in queryObj['leftJoin']){
							selfToSelfQb=selfToSelfQb.leftJoin(key,queryObj['leftJoin'][key][0],queryObj['leftJoin'][key][1],queryObj['leftJoin'][key][2])
						}
					}
					selfToSelfQb=selfToSelfQb
					.leftJoin(orm['models'][options['tableName']],'r','o.'+options['idField']+'=r.'+options['idParentField'])
					.groupBy('o.'+options['idField']);
				var isLoadChildren=reqObj.isLoadChildren?reqObj.isLoadChildren:(body.isLoadChildren?body.isLoadChildren:false);
				if(isLoadChildren){
					selfToSelfQb
					.where('o.'+options['idParentField']+'=:idParentField',{idParentField:(reqObj['id'] || reqObj[options['idField']])})
					...
					Promise.all([ptotal,prows]).then(function(res){
				...
				let selfToSelfQb=orm.getRepository(options['tableName'])
					...
					.select(queryObj['select'])
					.addSelect('count(r.'+options['idField']+')','ChildrenCount')
					.leftJoin(orm['models'][options['tableName']],'r','r.'+options['treeNodeField']+' like concat(o.'+options['treeNodeField']+',".%")')
					.groupBy('o.'+options['idField']);
				var isLoadChildren=reqObj.isLoadChildren?reqObj.isLoadChildren:(body.isLoadChildren?body.isLoadChildren:false);
				if(isLoadChildren){
					selfToSelfQb
					.where('o.'+options['treeNodeField']+' like :treeNodeField',{treeNodeField:reqObj[options['treeNodeField']]+'.%'})
					.andWhere('o.'+options['treeNodeField']+' not like :ntreeNodeField',{ntreeNodeField:reqObj[options['treeNodeField']]+'.%.%'})
					.orderBy(queryObj.order?queryObj.order:queryObj.orderBy)
					.getRawMany()
					.then(function(res){
						options.success(checkPaging(res,res.length));
			...
			//getManyAndCount
			orm.getRepository(options['tableName']).findAndCount(queryObj).then(function(res){
		...
		// 	options.error(err)
		// })
		orm.getRepository(options['tableName']).save(reqObj['delta']).then(function(res){
			options.success(res)
		}).catch(function(err){
			options.error(err)
		})
		...
		// 	options.error(err)
		// })
		orm.getRepository(options['tableName']).delete(reqObj['delta']).then(function(res){
			options.success(res)
		}).catch(function(err){
			options.error(err)
		})
			...
			tasks.push(new Promise(function(resolve,reject){
				...
				if(!idField){
					...
					tempPromise.then(function(){
						...
						var ormSave=orm.getRepository(options['tableName']).findOne(findOptions).then(function(data){
...
const fresh=function(options,orm){
    ...
    .values(options['data'])
    .orUpdate({conflict_target:options['conflict_target'], overwrite:options['updateOnDuplicate']})
    .execute()
...
const magicsave=function(options,orm){
	...
	var ormSave=orm.getRepository(options['tableName']).findOne(findObj).then(function(data){
		data && (options['data'][options.idField]=data[options.idField]);
		if(options['updateOnDuplicate']){
			for(var key in options['data']){
				if(key!=options['idField'] && options['updateOnDuplicate'].indexOf(key)<0 && options['conflict_target'].indexOf(key)<0){
		...
		if(ormSave[options.idField]){
			orm.getRepository(options['tableName']).save(ormSave).then(function(res){
			...
			orm.getRepository(options['tableName']).save(options['data']).then(function(res){
				if(functionObj['success']){
					functionObj['success'](ormSave)
				}
			}).catch(function(err){
