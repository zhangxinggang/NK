...
class Routers {
				...
				return item.stack.some(rt=>{
					...
					if(paramMethods.includes(ctx.method) && rt.regexp.test(url.parse(ctx.url).pathname)){
						if(!rt.nkr_params){
							...
							let filePath=join(rt.nkr_params.rootDir,normalizeSafe(decodeURIComponent(ctx.path).replace(rt.nkr_params.rootPath, "/")))
							try{
			...
			let routes=()=>{
				...
				return async function staticServe (ctx, next) {
					...
					if (methods.includes(ctx.method)) {
						try {
							done = await send(ctx, normalizeSafe(ctx.path.replace(opts.rootPath, "/")), opts)
						} catch (err) {
							if (err.status !== 404) {
								throw err
							}
	...
	loadDynamicRoutes() {
		...
		this.dynamicRouteDirs.map((item) => {
			...
			const router = new Router()
			router['nkr_auth']=item.auth
			glob.sync(join(item.rootDir, '/**/*.{m}.*js'),{ignore:item.ignore})
			.map((filterRoute) => {
				...
				if (routeType.length > 1) {
					...
					let purifyRoute = normalizeSafe(toUnix(filterRoute).replace(toUnix(item.rootDir), "/").replace(/\.\{(.+?)\}|\.js/g, ''))
