...
class NodeRtmpSession {
    constructor(socket,server) {
        ...
        this.outChunkSize = server.rtmpServer.chunk_size ? server.rtmpServer.chunk_size : RTMP_CHUNK_SIZE;
        this.pingTime = server.rtmpServer.ping ? server.rtmpServer.ping * 1000 : RTMP_PING_TIME;
        this.pingTimeout = server.rtmpServer.ping_timeout ? server.rtmpServer.ping_timeout * 1000 : RTMP_PING_TIMEOUT;
        ...
        this.gopCacheEnable = server.rtmpServer.gop_cache;
  ...
  onCreateStream(invokeMessage) {
    ...
    onPublish(invokeMessage) {
        ...
        if (this.server.rtmpServer.auth && this.server.rtmpServer.auth.publish && !this.isLocal && !this.isIPC) {
            let results = NodeCoreUtils.verifyAuth(this.publishArgs.sign, this.publishStreamPath, this.server.rtmpServer.auth.secret);
            if (!results) {
  ...
  onPlay(invokeMessage) {
    ...
    if (this.server.rtmpServer.auth && this.server.rtmpServer.auth.play && !this.isLocal && !this.isIPC) {
      let results = NodeCoreUtils.verifyAuth(this.playArgs.sign, this.playStreamPath, this.server.rtmpServer.auth.secret);
      if (!results) {
